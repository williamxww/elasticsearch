...
public class SearchServiceImpl implements SearchService {
    ...
    public void search() {
        SearchResponse searchResponse = client.getConnection().prepareSearch("twitter").setTypes("tweet")
                .setSearchType(SearchType.DFS_QUERY_THEN_FETCH).setQuery(QueryBuilders.matchQuery("name", "三胖子"))
                .setPostFilter(QueryBuilders.rangeQuery("age").from(19).to(400)).setFrom(0).setSize(20).setExplain(true)
    ...
    public void searchByCondition() throws Exception {
        ...
        SearchRequestBuilder searchRequestBuilder = client.getConnection().prepareSearch("red89").setTypes("test");
        ...
        // QueryBuilders.matchQuery("name", "中华");

        searchRequestBuilder.setQuery(functionScoreQuery(QueryBuilders.matchQuery("name", "中华").operator(Operator.AND),
        ...
        // boolQueryBuilder.must(getRangeFilterQuery("age",new
        // Integer[]{null,45}));
        ...
        // filterFunctionBuilders).boostMode(CombineFunction.SUM);

        // FunctionScoreQueryBuilder functionScoreQueryBuilder =
        // functionScoreQuery(boolQueryBuilder, scriptScoreFunctionBuilder);
        ...
        range.getBuckets().forEach(bucket -> {
            ...
            // bucket.getKey(),
            // bucket.getFrom(),bucket.getTo(),bucket.getDocCount());
            System.out.println(String.format("key [%s], from [%s], to [%s], doc_count [%d]", bucket.getKey(),
                    bucket.getFrom(), bucket.getTo(), bucket.getDocCount()));
        ...
        //
        // SearchRequestBuilder searchRequestBuilder1 =
        // client.getConnection().prepareSearch("red88")
        // .setTypes("test");
        //// QueryBuilder tel =
        // QueryBuilders.boolQuery().should(QueryBuilders.matchQuery("tel","18612855433")).should(QueryBuilders.matchQuery("message","程序设计"));
        //// BoolQueryBuilder filter1 =
        // QueryBuilders.boolQuery()//.must(QueryBuilders.matchQuery("age",
        // "40").operator(Operator.AND))
        ...
        // AggregationBuilders.terms("agg").field("attr_name");
        //
        // aggregationBuilder.size(100);
    ...
    public FunctionScoreQueryBuilder sortByFucntion(QueryBuilder queryBuilder) {
        FunctionScoreQueryBuilder query = functionScoreQuery(queryBuilder, ScoreFunctionBuilders
                .fieldValueFactorFunction("age").modifier(FieldValueFactorFunction.Modifier.LN1P).factor(1f))
    ...
    public void multiSearch() {
        ...
        MultiSearchResponse multiSearchResponse = client.getConnection().prepareMultiSearch().add(srb1).add(srb2).get();
    ...
    public void aggsearch() {
        ...
        SearchResponse response = requestBuilder.setQuery(QueryBuilders.matchQuery("name", "三胖"))
                .addAggregation(aggregationBuilder).addSort("age", SortOrder.DESC).setExplain(true).execute()
                .actionGet();
    ...
    public void metricsAgg() {
        ...
        SearchRequestBuilder searchRequestBuilder = client.getConnection().prepareSearch("hello").setTypes("test")
                .setFrom(0).setSize(100);
        ...
        SearchResponse searchResponse = searchRequestBuilder.setQuery(QueryBuilders.matchAllQuery())
                .addAggregation(sumAggregationBuilder).setExplain(true).execute().actionGet();
        ...
        SearchResponse searchResponse1 = searchRequestBuilder.setQuery(QueryBuilders.matchAllQuery())
                .addAggregation(statsAggregationBuilder).setExplain(true).execute().actionGet();
        ...
        ExtendedStatsAggregationBuilder extendedStatsAggregationBuilder = AggregationBuilders
                .extendedStats("extend_status").field("age");

        SearchResponse searchResponse2 = searchRequestBuilder.setQuery(QueryBuilders.matchAllQuery())
                .addAggregation(extendedStatsAggregationBuilder).setExplain(true).execute().actionGet();
